    ...
    public static class AdjustableExtensions
    {
        ...
        public static ExpandoObject ToExpando(this object obj, IEnumerable<string> fields)
        {
            ...
            foreach (var field in fields)
            {
                ...
                {
                        var ass = Assembly.Load("ArkFury.Entities");

                        var tpes = ass.GetTypes();
                        
                        var result = ass.GetTypes().SingleOrDefault(t => t.Name == field);
                        
                        PropertyInfo piTheList = obj.GetType().GetProperty(field); //Gets the properties

                        IList oTheList = piTheList.GetValue(obj, null) as IList;


                        dict[field] = JsonConvert.SerializeObject(oTheList);
                }
            }
